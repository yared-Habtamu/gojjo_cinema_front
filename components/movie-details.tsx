...
export function MovieDetails({ movie }: MovieDetailsProps) {
  ...
  useEffect(() => {
    ...
    const loadSimilar = async () => {
      // If backend provided a tmdb id, fetch similar movies from our TMDb proxy
      if (process.env.NEXT_PUBLIC_API_URL && (movie as any).tmdb_id) {
        try {
          ...
          console.warn(
            "Failed to load TMDb similar movies, falling back to mock",
            err
          );
        }
      }

      // fallback: leave similarMovies empty (avoid using mock-data)
    };
    void loadSimilar();
    return () => {
      mounted = false;
    ...
    const load = async () => {
      ...
      if (process.env.NEXT_PUBLIC_API_URL) {
        try {
          ...
          console.warn(
            "Failed to load comments from API, falling back to localStorage",
            err
          );
        }
      }
    ...
    const post = async () => {
      if (process.env.NEXT_PUBLIC_API_URL) {
        try {
          ...
          console.warn(
            "Failed to post comment to API, falling back to localStorage",
            err
          );
        }
      }

...
async function fetchCombinedByTitle(title: string) {
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000";
  const params = new URLSearchParams({ t: title });
  const res = await fetch(
    `${API_BASE.replace(/\/$/, "")}/api/movies/combined/?${params.toString()}`,
    { cache: "no-store" }
  );
...
export default async function MoviePage({ params }: MoviePageProps) {
  ...
  if (!movie && (tmdbPrefixMatch || tmdbNumericMatch)) {
    try {
      const tmdbId = tmdbPrefixMatch ? Number(tmdbPrefixMatch[1]) : Number(rawId);
